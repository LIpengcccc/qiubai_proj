...
class ToscrapeSpider(RedisSpider):
    name = 'toscrape'
    ...
    def parse(self, response):
        ...
        for li in li_list:
            ...
            yield Request(url=url_item, callback=self.parse_detail)
        ...
        if next_page is not None:
            if next_page.startswith('catalogue/'):
                ...
                next_url = "http://books.toscrape.com/" + 'catalogue/' + next_page
            # print(next_url)
            # self.log(f"start to scrapy next_url:{next_url}", level=logging.INFO)
            r = Request(url=next_url, dont_filter=True)
            yield r
